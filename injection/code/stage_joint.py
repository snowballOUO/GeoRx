...
from h5_score_interaction import anchor_ids, contract_queries, decompose, four_point_rms
...
def construct(q0, p0, pair, clean_idx, axis, seed, clean_train):
    ...
    if "h5" in pair:
        q_before = q.copy()
        q = contract_queries(q, clean_train, t)
        meta["h5_transport"] = "clean_train_mean_query_contraction"
        meta["h5_query_cosine_to_pre"] = float(np.mean(np.sum(q*q_before, axis=1)))
...
def audit(q0, p0, q, p, pair, meta, clean_vals, clean_idx, device, batch_q):
    ...
    for k in pair:
        ...
        if k == "h1":
            ...
            anchors = anchor_ids(len(p))
            before_share = decompose(q0 @ p0[anchors].T)["interaction_share"]
            after_share = decompose(q @ p[anchors].T)["interaction_share"]
            ratio = after_share / max(before_share, 1e-12)
            mask = np.full(len(q), after_share < before_share, dtype=bool)
            valid = bool(after_share < before_share - 1e-8)
            extra = {"clean_interaction_share": float(before_share),
                     "final_interaction_share": float(after_share),
                     "interaction_share_ratio": float(ratio)}
        flags[k] = mask & valid
...
def run_record(ctx, cell, seed, pair, order, inp, profile, cfg, clean_idx, axis):
    ...
    q, p, meta = construct(q0, p0, pair, clean_idx, axis, seed, inp["train_q"])
